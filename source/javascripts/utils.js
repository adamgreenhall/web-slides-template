(function() {
  ...
  var SlideShow = function(slides) {
    ...
    this.current = (!this.current) ? "start" : this.current.replace('#', '');
    if (!query('#' + this.current)) {
      // if this happens is very likely that someone is coming from
      // a link with the old permalink format, i.e. #slide24
      console.log('The format of the permalinks have recently changed. If you are coming ' +
  ...
  SlideShow.prototype = {
    ...
    _update: function(targetId, dontPush) {
      ...
      if (elem && elem.className != 'presentation') {
        ...
        console.log(this._presentationCounter)
        console.log(this._presentationCounter.textContent)  
        this._presentationCounter.textContent = currentIndex;
        if (this._menuCounter) {
          this._menuCounter.textContent = currentIndex;          
        }
      }
      var speakerNote = this._slides[currentIndex - 1].getSpeakerNote();
      if (speakerNote){this._speakerNote.innerHTML = speakerNote};
